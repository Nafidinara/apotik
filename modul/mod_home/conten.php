  ...
  Highcharts.chart('container', {
    ...
    <section class="content">
      ...
      <div class="row">
        ...
        // $d = mysqli_query($connection,"select sum(harga * jumlah) as total from detail_transaksi where id_transaksi 
				// 						in(select id_transaksi from transaksi where created_user='12345' and tgl_beli= CURDATE())
        // 					");
        $tgl = date('Y-m-d');
        $d = mysqli_query($connection,"SELECT * FROM detail_transaksi WHERE tanggal='$tgl'");
          $h = mysqli_fetch_array($d);
          
          foreach($d as $hsum){
            if($hsum['diskon'] == '0'){
                  $sum += $hsum['harga'] * $hsum['jumlah'];
          }else{
                $diskon = $hsum['diskon']/100;
                $sum += ($hsum['harga'] * $hsum['jumlah']) * $diskon;
          }
        }
					echo "$sum"; ?>
			</span>
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
          <div class="info-box border">
            ...
            <span class="info-box-text" style="
													...
													font-family: cursive;
												">
			    Total Penjualan Hari Ini</span>
              <span class="info-box-number" style="font-size: 30px;">
              <?php 
              $d1 = mysqli_query($connection,"SELECT * FROM detail_transaksi WHERE tanggal='$tgl'");
