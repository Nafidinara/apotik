...
$r=mysqli_fetch_array(mysqli_query($connection,"SELECT * FROM admins WHERE nik='$_SESSION[nik]'"));
...
// Apabila password lama cocok dengan password admin di database
  ...
  if ($_POST['pass_baru']==$_POST['pass_ulangi']){
    ...
    mysqli_query($connection,"UPDATE admins SET password = '$pass_baru',
                                   exprd    = '$limit'
...
elseif ($module=='password' AND $act=='tambahatasan'){
  $nik=$_POST['nik'];
  $connection = mysqli_connect("localhost", "root", "","apotik");
    mysqli_query($connection,"INSERT INTO email (nik,
                                    email) 
                            VALUES('$_POST[nik]',
                                   '$_POST[email]') ");
...
//Hapus Email Atasan
  elseif ($module=='password' AND $act=='hapusemail'){
    mysqli_query($connection,"DELETE FROM email WHERE id='$_GET[id]'");
