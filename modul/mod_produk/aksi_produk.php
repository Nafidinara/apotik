...
if (empty($_SESSION['nik']) AND empty($_SESSION['passuser'])){
  ...
  // $codePerusahaan = 1;
    ...
    if ($module=='produk' AND $act=='input'){
      ...
      $insert = mysqli_query($connection,"INSERT INTO `produk` (
											      `nm_barang`,
					 ...
					 //mengambil id_terakhir                               
						...
						$count_P = count($kategori_harga);
						 ...
						 $jumlah_pcs = $_POST['jumlah_pcs'];
						   ...
						   for($i=0;$i<$count_P;$i++){   
							 $insert = mysqli_query($connection,"INSERT INTO `harga` (`id_produk`, 
																		   `kategori_harga`, 
																		   `harga`, 
																		   `jumlah_pcs`
																		   ) 
    ...
    elseif ($module=='produk' AND $act=='delete'){
          ...
          $insert = mysqli_query($connection,"DELETE FROM `produk` WHERE kd_obat='$kd_obat' ");
