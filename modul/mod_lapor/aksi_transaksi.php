...
if (empty($_SESSION['nik']) AND empty($_SESSION['passuser'])){
  ...
  $act=$_GET['act'];
   ...
   date_default_timezone_set("Asia/Jakarta");
    ...
    if ($module=='lapor' AND $act=='inputPrint'){
		if (empty($_POST['id_transaksi']) || empty($_POST['kasir']) ){
			header('location:../../media.php?module=lapor');
			...
			$insert = mysqli_query($connection,"INSERT INTO `transaksi` (`id_transaksi`,
				...
				for ($i = 0; $i < $jml; $i++){
				  mysqli_query($connection,"INSERT INTO detail_transaksi(id_transaksi, kd_obat, nm_produk, jenis, harga, jumlah, diskon,id_harga,jumlah_pcs)
				...
				// if($insert) {
				   // tambahlog($nik,$module,'tambah order temp','BERHASIL');
					  // echo "<script type='text/javascript'>
						...
						// </script>";
				  // }
				  header('location:../../modul/mod_lapor/cetak_epson.php?act=cetak_transaksi&id_transaksi='.$id_transaksi.'');
        }

      }

    // delete module jabatan
    elseif ($module=='lapor' AND $act=='delete'){
		$connection = mysqli_connect("localhost", "root", "","apotik");
